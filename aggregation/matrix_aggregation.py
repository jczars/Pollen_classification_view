...
def sum_and_plot_confusion_matrices(folder_path, output_csv_path, output_image_path, normalize=False):
    ...
    sns.heatmap(mat, cmap="Blues", annot=True, fmt="d",  # Force integer display
            xticklabels=categories, yticklabels=categories, cbar=True, ax=ax, linewidths=0.5)

    # Set axis labels and title
    ax.set_xlabel('Predicted Labels', fontsize=12)
    ax.set_ylabel('True Labels', fontsize=12)
    ax.set_title('Summed Confusion Matrix' + (' (Normalized)' if normalize else ''), fontsize=14)
...
def performance_report_pd(arr, classes, save_dir, id_test, nm_model):
...
def saved(folder):
    ...
    save_dir = os.path.join(
        ...
        os.path.basename(folder.rstrip('/')) + '_aggregated/'  # Add '_aggregated' to the folder name
    )
...
if __name__ == "__main__":
    # Example usage:
    folder = '/home/jczars/Pollen_classification_view/results/phase2/Reports/1_DenseNet201_reports/'
    save_dir = saved(folder)
    mat_csv = os.path.join(save_dir, 'aggregated_confusion_matrix.csv')
    mat_image = os.path.join(save_dir, 'aggregated_confusion_matrix.png')
    output_reports_csv = os.path.join(save_dir, 'aggregated_classification_report.csv')


    # Process confusion matrices and generate the report
    conf_matrix_df = sum_and_plot_confusion_matrices(folder, mat_csv, mat_image, normalize=False)
