...
def run(params):
    ...
    for view in views:
        src_view = os.path.join(params['bd_src'], view)
        categories = sorted(os.listdir(src_view))
        dst_view = os.path.join(params['bd_src'], f"{view}_R")
        copy_base(src_view, dst_view, params['nm_new_base'], categories, params['k'], params['bd_src'], view)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", 
        type=str, 
        default="/media/jczars/4C22F02A22F01B22/$WinREAgent/Pollen_classification_view/1_create_bd/config_split.yaml",
