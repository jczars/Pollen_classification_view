...
import glob
import imageio.v2 as imageio
from imgaug import augmenters as iaa, parameters as iap
from models import utils
...
def run_balancing(dst_base, categories, goal, img_type, k_folds, save_dir, verbose=0):
    ...
    for i in range(k_folds):
        ...
        for category in categories:
            ...
            calculate_augmentation(goal, len(images), images, img_type, save_dir=category_path, verbose=verbose)
...
def process_augmentation(config):
    ...
    for vista in vistas:
        ...
        run_balancing(dst_base, categories, goal, tipo, k_folds, save_dir, verbose=verbose)
